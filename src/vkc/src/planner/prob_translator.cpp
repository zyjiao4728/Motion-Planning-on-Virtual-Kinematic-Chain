...
ProbTranslator::ProbTranslator(OmplPlanParameters params) {
  ...
  // srand((unsigned int)time(NULL));   // if this expression is commented on,
...
tesseract_planning::JointWaypointPoly ProbTranslator::setupStartWaypoint(
  ...
  auto current_state = env.getVKCEnv()->getTesseract()->getState();
  Eigen::VectorXd start_pos;
  start_pos.resize(kin_->numJoints());
  int joint_num = 0;
  // JointWaypoint(std::vector<double> joint_positions, std::vector<std::string>
  // joint_names)
...
tesseract_kinematics::KinematicGroup::ConstPtr ProbTranslator::getKinematics() {
  return kin;
}
...
void ProbTranslator::setupParams(VKCEnvBasic &env, vkc::ActionBase::Ptr act) {
  ...
  this->kin = env.getVKCEnv()->getTesseract()->getKinematicGroup(
      act->getManipulatorID());

  // // there has got to be a better way to do this
  // this->kin_base =
  // env.getVKCEnv()->getTesseract()->getFwdKinematicsManager()->getFwdKinematicSolver("base");
  // this->kin_arm =
  // env.getVKCEnv()->getTesseract()->getFwdKinematicsManager()->getFwdKinematicSolver("arm");

  return;
}

bool ProbTranslator::solveProblem(PlannerResponse &response,
  ...
  // boost::optional<ompl::geometric::PathGeometric> maybe_path =
  // coi_->plan(start_waypoint.getPositions(), goal_waypoint->getPositions(),
  // params_);

  // if (maybe_path)
  // {
  //   response.data =
  //   std::make_shared<tesseract_common::TrajArray>(vkc::toTrajArray(*maybe_path));
  ...
  //     res_traj.push_back(std::vector<double>(begin, begin +
  //     response.trajectory.cols()));
  //   }
...
tesseract_kinematics::IKSolutions ProbTranslator::inverseKinematics(
  ...
  tesseract_kinematics::KinGroupIKInput ik_input(
      pose, "world", kin_->getAllPossibleTipLinkNames()[0]);
  return kin_->calcInvKin(ik_input, seed);
}

void ProbTranslator::getJointNameIndexMap(
...
bool ProbTranslator::collisionFreeInverseKinematics(
    VKCEnvBasic &env, std::string manipulator, Eigen::VectorXd &solutions,
    const Eigen::Isometry3d &pose, Eigen::VectorXd &seed) {
  // auto inv_kin_mgr =
  // env.getVKCEnv()->getTesseract()->getInvKinematicsManager(); auto
  // inv_kin_solver = inv_kin_mgr->getInvKinematicSolver(manipulator);
  auto kin_ = env.getVKCEnv()->getTesseract()->getKinematicGroup(manipulator);
  if (kin_) {
    // ROS_INFO("[%s]it will try %d times to get a collision free waypoint, ik
    // solver name: %s",
    //          __func__, inv_attp_max_, kin_->getsolvername() c_str());

    solutions.resize(kin_->numJoints());
    seed.resize(kin_->numJoints());

    auto state_og = env.getVKCEnv()->getTesseract()->getState();
    auto adj_map = env.getVKCEnv()->getSceneGraph()->getAdjacencyMap(
        kin_->getActiveLinkNames());

    auto collision_manager =
        env.getVKCEnv()->getTesseract()->getDiscreteContactManager()->clone();
    collision_manager->setActiveCollisionObjects(kin_->getActiveLinkNames());

    int attempts = 0;
    bool satisfied = false;
    tesseract_collision::ContactResultMap collisions;
    tesseract_common::KinematicLimits joint_limits = kin_->getLimits();

    while (attempts < inv_attp_max_) {
      ROS_WARN("[%s]%d times try to get a collision free waypoint", __func__,
               attempts);
      ...
      if (!solutions.empty() && checkJointLimits(solutions[0], joint_limits)) {
        auto state = env.getVKCEnv()->getTesseract()->getState(
            kin_->getJointNames(), solutions[0]);

        collision_manager->setCollisionObjectsTransform(state.link_transforms);

        collisions.clear();
        collision_manager->setDefaultCollisionMarginData(
            0.01);  // wanglei@2021-11-12
        collision_manager->contactTest(
            collisions, tesseract_collision::ContactTestType::FIRST);
        ...
        if (0 == collisions.size()) {
          ...
          ROS_INFO(
              "[%s]the solution results in at least one collision:\n\t%s "
              "<--------> %s",
    ...
    ROS_ERROR("[%s]failed to find a collision free state after %d attempts.",
              __func__, attempts);
    return false;
  } else {
    ROS_ERROR("[%s]Unable to find inverse kinematics solver for  %s.", __func__,
              manipulator.c_str());
    return false;
  }
}

void ProbTranslator::genRandState(
    tesseract_kinematics::KinematicGroup::UPtr &kin, Eigen::VectorXd &seed) {
  ...
  for (auto &jnt : joint_name_idx) {
    ...
    seed[jnt.second] =
        joint_limits(jnt.second, 0) +
        f * (joint_limits(jnt.second, 1) - joint_limits(jnt.second, 0));
  }
}

bool ProbTranslator::getStartAndGoalState(VKCEnvBasic &env,
...
bool ProbTranslator::checkJointLimits(
    const Eigen::VectorXd &solution,
    const tesseract_common::KinematicLimits limits) {
  for (long int i = 0; i < solution.size(); ++i) {
    if (solution[i] < limits.joint_limits(i, 0) ||
        solution[i] > limits.joint_limits(i, 1)) {
      ROS_INFO("[%s]joint %ld exceeds limits, expected: [%f, %f], actual: %f",
               __func__, i + 1, limits.joint_limits(i, 0),
               limits.joint_limits(i, 1), solution[i]);
      return false;
    }
  }
  return true;
...
bool ProbTranslator::transPickProb(VKCEnvBasic &env, vkc::PickAction::Ptr act) {
  ...
  setupParams(env, act);
  // this->coi_ =
  // std::make_shared<ChainOmplInterface>(env.getVKCEnv()->getTesseract(),
  // this->kin); this->coi_->setAdjacencyMap();
...
bool ProbTranslator::transPlaceProb(VKCEnvBasic &env,
                                    vkc::PlaceAction::Ptr act) {
...
bool ProbTranslator::transGotoProb(VKCEnvBasic &env, vkc::GotoAction::Ptr act) {
  ...
  // std::make_shared<tesseract_motion_planners::ContinuousMotionValidator>(
  //     this->coi_->spaceInformation(),
  //     env.getVKCEnv()->getTesseract(),
  //     kin);
  ...
  if (act->getLinkObjectives().size() && act->getJointObjectives().size()) {
    ROS_ERROR(
        "Both link objectives and jointobjectives found, using link objectives "
        "only ");
  }
...
// void ProbTranslator::setBaseBias(double reach_length, double clearance,
// double span, double bias)
// {
...
//   {
//     ROS_INFO("Full IK");
//     ewp_std = IKhelper(env, this->l_obj, this->j_obj, map, n_steps,
//     joint_limits, init_type); break;
//   }
...
// void ProbTranslator::randBase(Eigen::VectorXd &goal_pose, Eigen::VectorXd
// &res, BaseBias &bias)
// {

//   double len = (double)rand() / RAND_MAX * (bias.reach_length -
//   bias.clearance) + bias.clearance; double ang = (double)rand() / RAND_MAX *
//   bias.span - bias.span * 0.5 + bias.bias;
...
//   // while (!init_base_position) { init_base_position = true; double r =
//   (rand() % 1000) / 1000.0 * 0.8;
...
//   contact_results, tesseract_collision::ContactTestType::ALL); for (auto
//   &collision : contact_results) { if (collision.first.first == "base_link" ||
//   collision.first.second == "base_link") { init_base_position = false; break;
//   } } contact_results.clear(); }
// }

// Eigen::VectorXd ProbTranslator::genBaseGoal(VKCEnvBasic &env,
// std::vector<LinkDesiredPose> &link_objectives, BaseBias &bias)
// {
//   // std::vector<double> res;
//   Eigen::VectorXd res;

//   const int max_tries = 100;
//   EnvState::ConstPtr current_state =
//   env.getVKCEnv()->getTesseract()->getCurrentState();
...
//         env.getVKCEnv()->getTesseract()->getState(this->kin_base->getJointNames(),
//         base_pose);

//     contact_results.clear();
//     disc_cont_mgr_->setCollisionObjectsTransform(env_state->transforms);

//     disc_cont_mgr_->contactTest(contact_results,
...
// Eigen::VectorXd ProbTranslator::initArmGoal(VKCEnvBasic &env,
...
//   env.getVKCEnv()->getTesseract()->setState(base_joint_names, base_pose);

//   tesseract::InverseKinematicsManager::Ptr inv_kin_mgr =
//   env.getVKCEnv()->getTesseract()->getInvKinematicsManager();
//   tesseract_collision::DiscreteContactManager::Ptr disc_cont_mgr_ =
//       env.getVKCEnv()->getTesseract()->getDiscreteContactManager()->clone();
...
//   std::vector<std::string> joint_names;
//   joint_names =
//   inv_kin_mgr->getInvKinematicSolver(arm_name)->getJointNames();
//   total_joint_names.reserve(base_joint_names.size() + joint_names.size()); //
//   preallocate memory total_joint_names.insert(total_joint_names.end(),
//   base_joint_names.begin(), base_joint_names.end());
//   total_joint_names.insert(total_joint_names.end(), joint_names.begin(),
//   joint_names.end());
...
//       if (link_obj.link_name ==
//       inv_kin_mgr->getInvKinematicSolver(arm_name)->getTipLinkName())
//       {
...
//         inv_kin_mgr->getInvKinematicSolver(arm_name)->getLimits();
...
//               {
//                 seed[jnt.second] = rand() % (int(joint_limits(jnt.second,
//                 1))) - int(0.5 * joint_limits(jnt.second, 1));
//               }
//               else
//               {
//                 seed[jnt.second] = rand() % (2 * int(joint_limits(jnt.second,
//                 1))) - int(joint_limits(jnt.second, 1));
//               }
//             }
//           }
...
//           inv_suc =
//           inv_kin_mgr->getInvKinematicSolver(arm_name)->calcInvKin(sol,
//           link_obj.tf, seed);

//           // std::cout << "IK solver name is"<< std::endl;
//           // std::cout <<
//           inv_kin_mgr->getInvKinematicSolver(arm_name)->getSolverName().c_str()
//           << std::endl;
//           // tesseract_kinematics::KDLInvKinChainLMA::Ptr con_ptr =
//           std::dynamic_pointer_cast<tesseract_kinematics::KDLInvKinChainLMA>(inv_kin_mgr->getInvKinematicSolver(arm_name));
...
//           tesseract_environment::EnvState::Ptr env_state =
//               env.getVKCEnv()->getTesseract()->getState(joint_names, sol);
...
//           // if (!satisfy_collision) std::cout <<  "has collision" <<
//           std::endl;

//           satisfy_limit = checkJointLimit(sol,
//           inv_kin_mgr->getInvKinematicSolver(arm_name)->getLimits(),
...
//       else
//       {
//         ROS_WARN("Unsupported trajectory initialization for given link
//         desired pose:\t%s", link_obj.link_name.c_str());
//       }
//     }
//   }
...
// Eigen::VectorXd ProbTranslator::optArmGoal(VKCEnvBasic &env,
// std::vector<LinkDesiredPose> &link_objectives,
//                                            std::vector<JointDesiredPose>
//                                            &joint_objectives, Eigen::VectorXd
//                                            base_pose)
// {
//   Eigen::VectorXd res;
...
// TrajOptProb::Ptr ProbTranslator::moveArm(VKCEnvBasic &env, int n_steps,
// Eigen::Vector3d p_goal, Eigen::Quaterniond q_goal, Eigen::VectorXd base_pose)
// {
...
//   // if (!validateGroupID(env.getVKCEnv()->getTesseract(),
//   a_pci.basic_info.manip))
//   //   exit(1);
...
//   a_pci.init_info.data =
//   start_waypoint.transpose().replicate(a_pci.basic_info.n_steps, 1);

//   // Populate Cost Info
//   std::shared_ptr<JointVelTermInfo> jv =
//   std::shared_ptr<JointVelTermInfo>(new JointVelTermInfo); jv->coeffs =
//   std::vector<double>(joint_num, 5.0); jv->targets =
//   std::vector<double>(joint_num, 0.0); jv->first_step = 0; jv->last_step =
//   a_pci.basic_info.n_steps - 1; jv->name = "joint_vel"; jv->term_type =
//   TT_COST; a_pci.cost_infos.push_back(jv);

//   std::shared_ptr<CollisionTermInfo> collision =
//   std::shared_ptr<CollisionTermInfo>(new CollisionTermInfo); collision->name
//   = "collision"; collision->term_type = TT_CNT; collision->continuous = true;
//   collision->first_step = 0;
//   collision->last_step = a_pci.basic_info.n_steps - 2;
//   collision->gap = 1;
//   collision->info = createSafetyMarginDataVector(a_pci.basic_info.n_steps,
//   0.0001, 40); a_pci.cnt_infos.push_back(collision);

//   std::shared_ptr<CartPoseTermInfo> ee_goal =
//   std::shared_ptr<CartPoseTermInfo>(new CartPoseTermInfo); ee_goal->term_type
//   = TT_CNT; ee_goal->name = "ee_goal"; ee_goal->link = "ur_arm_ee_link";
//   ee_goal->timestep = a_pci.basic_info.n_steps - 1;
//   ee_goal->xyz = p_goal;
//   ee_goal->wxyz = Eigen::Vector4d(q_goal.w(), q_goal.x(), q_goal.y(),
//   q_goal.z()); ee_goal->pos_coeffs = Eigen::Vector3d(10, 10, 10);
...
// std::vector<double> ProbTranslator::BaseFirstIK(VKCEnvBasic &env,
// std::vector<LinkDesiredPose> &link_objectives,
//                                                 std::vector<JointDesiredPose>
//                                                 &joint_objectives, BaseBias
//                                                 &bias)
// {
...
//   //
//   env.getVKCEnv()->getTesseract()->setState(this->kin_base->getJointNames(),
//   base_pose);
//   // EnvState::ConstPtr state =
//   env.getVKCEnv()->getTesseract()->getCurrentState();
...
//   response.trajectory.row(response.trajectory.rows() - 1);

//   // Eigen::VectorXd arm_pose = initArmGoal(env, link_objectives,
//   joint_objectives, base_pose);
...
//   env.getVKCEnv()->getTesseract()->setState(this->kin->getJointNames(),
//   start_waypoint);
...
// std::vector<double> ProbTranslator::IKhelper(VKCEnvBasic &env,
// std::vector<LinkDesiredPose> &link_objectives,
//                                              std::vector<JointDesiredPose>
//                                              &joint_objectives, MapInfo map,
//                                              int n_steps, Eigen::MatrixX2d
//                                              &joint_limits, InitInfo
//                                              init_type)
// {
//   int inv_attp = 0;
//   ROS_INFO("Determining Full IK");
//   std::vector<double> ewp_std = HuskeyIK(env, link_objectives,
//   joint_objectives, map, n_steps, joint_limits);
//   // while (!(this->coi_->isStateValidStd(ewp_std)))
//   // {
//   //   ROS_INFO("Retrying IK");
//   //   ewp_std = HuskeyIK(env, link_objectives, joint_objectives, map,
//   n_steps, joint_limits);
